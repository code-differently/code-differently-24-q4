...
export class LibraryFactory {
     ...
     * @throws IOException
     */
    static async createWithLoader(loader: LibraryDataLoader): Promise<LibraryService> {
        ...
        const checkoutsByEmail: Map<string, CheckoutModel[]> = data.getCheckoutsByEmail();
        const mediaItemById: Map<string, MediaItem> = this.getMediaItemsById(mediaItems);
        const guestsByEmail: Map<string, LibraryGuest> = this.getGuestsByEmail(guests);
        this.checkOutItems(library, checkoutsByEmail, guestsByEmail, mediaItemById);

        return library;
    }

    private static getMediaItemsById(mediaItems: MediaItem[]): Map<string, MediaItem> {
    ...
    private static addLibraryGuests(library: LibraryService, guests: LibraryGuest[]): void {
        for (const guest of guests) {
            library.addLibraryGuest(guest);
        }
    }

    private static addLibraryItems(library: LibraryService, mediaItems: MediaItem[], firstLibrarian: Librarian): void {
        for (const mediaItem of mediaItems) {
            library.addMediaItem(mediaItem, firstLibrarian);
        }
    }

    private static getGuestsByEmail(guests: LibraryGuest[]): Map<string, LibraryGuest> {
