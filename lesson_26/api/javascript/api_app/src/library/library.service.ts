...
export class LibraryService {
    ...
    getLibrarians(): Set<Librarian> {
        return new Set(
            Array.from(this.guestsById.values()).filter((g): g is Librarian => g instanceof Librarian)
        );
    }
    ...
    hasLibraryGuestByEmail(emailAddress: string): boolean {
        return Array.from(this.guestsById.values()).some(g => g.getEmail().toLowerCase() === emailAddress.toLowerCase());
    }
