...
import { Book, Dvd, Magazine, MediaItem, MediaType, Newspaper } from "../library";
...
export function asMediaItem(request: MediaItemRequest): MediaItem {
    const id = request.id ? request.id : randomUUID();
    switch (request.type.toLowerCase()) {
        case MediaType.BOOK:
            return new Book(id, request.title, request.isbn, request.authors, request.pages);
        case MediaType.DVD:
