...
import { asMediaItem } from './media_item_request';
...
export class MediaItemsController {
    ...
    getItems(): GetMediaItemsResponse {
        ...
        const responseItems: MediaItemResponse[] = [...items].map(toMediaItemResponse);
        const response = { items: responseItems };
        return response;
    }

    @Get('items/:id')
    getItem(@Param('id') id: string, @Res({ passthrough: true }) res: Response): MediaItemResponse {
    ...
    addItem(@Body() body: CreateMediaItemRequest, @Res({ passthrough: true }) res: Response): CreateMediaItemResponse {
        if (!body.item) {
            res.status(HttpStatus.BAD_REQUEST);
            return { errors: ['Missing item'] };
        }
        const item = asMediaItem(body.item);
    ...
    deleteItem(@Param('id') id: string, @Res({ passthrough: true }) res: Response): void {
